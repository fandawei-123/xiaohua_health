...
import com.huahua.constant.RedisMessageConstant;
import com.huahua.dao.*;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Result addPhoneOrder(Map map) throws Exception {
        ...
        order.setAddress_id((Integer) map.get("dept"));//预约地点
    ...
    public void edit(Integer setmealId, Map map) {
        ...
        order.setAddress_id(addressDao.findIdByAddressName((String) map.get("dept")));
     ...
     */
    @Override
    public Map<String, Object> findSuggestionById(Integer id) {
        Order result = orderDao.findHealtMessageById(id);
        ...
        if (result.getFood() != null) {
            ...
            for (String f : foodArr) {
                foods.add(foodDao.findIdByFoodName(f));
            }
            map.put("foods", foods);
        }
        if (result.getSport() != null) {
            String sport = result.getSport();
            String[] sportArr = sport.split(",");
            for (String s : sportArr) {
                sports.add(sportDao.findIdBySportName(s));
            }
     ...
     */
    @Override
    public Object findByPhoneOrName(String queryString) {
        Page page = orderDao.findByPageAndCondition(queryString, null, null, null, null);
        //查询的总条数
        long total = page.getTotal();
        //查询的当前页的集合
        List result = page.getResult();
